...
##getting accuarte value using accurate variable
interface_name=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
hostname=$(hostname)
ipAddress=$(ip a s $interface_name |awk '/inet /{gsub(/\/.*/,"");print $2}')
LAN_Address=$(ip a s $interface_name |awk '/inet /{gsub(/\/.*/,"");print $2}')
LAN_Host=$(getent hosts $ipAddress | awk '{print $2}')
ExternalIP=$(curl -s icanhazip.com)
myExternalName=$(getent hosts $ExternalIP | awk '{print $2}')
routerName=$(route -n | grep '^0.0.0.0' | awk '{print $2}')
routerIP=$(getent hosts $routerName | awk '{print $2}')
